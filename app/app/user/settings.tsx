...
export default function SettingsScreen() {
    ...
    const settingDisplay = [
		{title: 'profile', data:
			...
			{title: "password", type: "to-page"},
			]},
		{title: 'friends', data:
			[{title: 'friends', type: 'to-page'},
			]},
                    ...
                    return (
                        <View>
                            <View style={{flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', height: 35, paddingHorizontal: 15,}}>
                                <View style={{flex: 1, flexDirection: "row", alignItems: 'center'}}><Text style={{fontSize: 16, color: "#2A3D45",}}>{item.title} </Text>
                                    {item.detail ? <TouchableOpacity onPress={() => {setShowDetail(showDetail == item.title ? "" : item.title)}}><Feather style={{fontSize: 16, color: '#2a3d45', padding: 7}} name='help-circle'/></TouchableOpacity> : <></>}
                                    {item.type == 'to-page' ? <TouchableOpacity><Feather style={{fontSize: 16, color: '#2a3d45', padding: 7,}} name='arrow-right'/></TouchableOpacity> : <></>}
                                    {item.type == 'checkbox' ? <TouchableOpacity activeOpacity={0.6} onPressOut={() => setTempSettings({...tempSettings, [item.setting ?? item.title]: !tempSettings[item.setting ?? item.title]})}><Feather style={{fontSize: 16, color: '#2a3d45', padding: 7,}} name={tempSettings[item.setting ?? item.title] == false ? "square" : "check-square"}/></TouchableOpacity> : <></>}
                                </View>
                                
                                {item.type == 'num-input' ? <View style={{flex: 0.6}}><TextInput value={`${tempSettings[item.setting ?? item.title]}`} onChangeText={(text) => {setTempSettings({...tempSettings, [item.setting ?? item.title]: text})}} maxLength={6} keyboardAppearance={tempSettings.darkMode ? "dark" : "light"} inputMode='decimal' style={{backgroundColor: 'white', width: 60, height: 25, borderWidth: 1.5, borderColor: "#467599", padding: 3,}}></TextInput></View> : <></>}
                                {item.type == 'text-input' ? <View style={{flex: 0.6}}><TextInput value={`${tempSettings[item.setting ?? item.title]}`} onChangeText={(text) => {setTempSettings({...tempSettings, [item.setting ?? item.title]: text})}} keyboardAppearance={tempSettings.darkMode ? "dark" : "light"} inputMode='decimal' style={{backgroundColor: 'white', width: 60, height: 25, borderWidth: 1.5, borderColor: "#467599", padding: 3,}}></TextInput></View> : <></>}
